...
(defrecord rocksdbStore [db path default-serializer serializers compressor encryptor read-handlers write-handlers locks]
...
(defn new-rocksdb-store
  [path & {:keys [rocksdb-opts default-serializer serializers compressor encryptor read-handlers write-handlers]
                :or {rocksdb-opts {}
  ...
  (let [res-ch (async/chan 1)]
    (async/thread 
      (try
        (let [db (rdb/create-db path rocksdb-opts)]
          (async/put! res-ch
            (map->rocksdbStore {:db db
                                :path path
